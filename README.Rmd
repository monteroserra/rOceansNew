...
invisible(Acropora_div = oceanDiversity(occurrences = Acropora_Total_Checked))
...
## Function 2.4 *oceanHotspots* 
This function allows classifiying sites into high, mid, and low diversity given any biodiversity metric
```{r}
...
# Functions 3: 
## Accessing and exploring current and future trends in marine environmental data  
...
# we can also analyze environmental drivers of species richness using previously computed grids 
abundance_environ_df = oceanEnv(biod_grid = Acropora_richness,
                       biodiv_metric = "richness", 
                       plot=T, 
                       log_scale=T)

# you can specify any alternative environmental parameters using env_parameters
...
## Example on sharks from the genus Carharhinidae

```{r}

#Download dataset on Carharhinidae (it has been previously checked and filtered using oceanDataCheck()
                                   
data(Carcharhinidae_total_checked) 

#Compte a abundance grid
Carcharhinidae_abundance = oceanAbundGrid(occurrences = Carcharhinidae_total_checked, cell_size=5)

Carcharhinidae_abundance_1x1 = oceanAbundGrid(occurrences = Carcharhinidae_total_checked, cell_size=1)

#Visualize abundance
oceanMaps(Carcharhinidae_abundance, logScale=T, main="Carcharhinidae abundance (5ºx5º)")

oceanMaps(Carcharhinidae_abundance_1x1, logScale=T, main="Carcharhinidae abundance (1ºx1º)")


# Exploring envioronmental drivers of abundance patterns
invisible(abundance_environ_df = oceanEnv(biod_grid = Carcharhinidae_abundance_1x1,
                                biodiv_metric = "abundance", 
                                plot=T, 
                                log_scale=T))
...
invisible(Carcharhinidae_diversity = oceanDiversity(occurrences = Carcharhinidae_total_checked))

# Visualizing richness
oceanMaps(Carcharhinidae_diversity[[2]], main = "Carcharhinidae richness")

# Exploring envioronmental drivers of abundance patterns
invisible(diversity_environ_df = oceanEnv(biod_grid = Carcharhinidae_diversity[[2]],
                                biodiv_metric = "richness", 
                                plot=T, 
                                log_scale=T))

invisible(oceanHotspots(biodiversity_grid = diversity_environ_df[[2]]))
